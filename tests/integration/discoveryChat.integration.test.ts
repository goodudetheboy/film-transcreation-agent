...
describe('frontend discoveryChatApiClient -> real backend -> mock discovery chat agent', () => {
  ...
  it('kicks off an Agent thread, logs a run inline, and the mock chat agent merges the candidate for real', async () => {
    ...
    const film = await createFilm(
      { title: 'Chat Film', videoUrl, subtitleUrl, subtitleFormat: format, subtitleEntries: entries, runDiscovery: false, testMode: true },
      { baseUrl: backend.url },
    );

    // ---- Start the Agent thread, kick off Run #1 from inside it ----
    const session = await createDiscoveryAgentSession(film.id, {}, { baseUrl: backend.url });
    expect(session.agentNumber).toBe(1);

    const job = await createDiscoveryJob(
      film.id,
      { agentNumber: session.agentNumber, specialInstruction: 'find something', targetColumns: ['segmentDescription'], testMode: true },
      { baseUrl: backend.url },
    );
    expect(await worker.processOne()).toBe(true);

    const loggedSession = await logDiscoveryRun(film.id, session.id, { jobId: job.id }, { baseUrl: backend.url });
    ...
    await sendDiscoveryChatMessage(film.id, session.id, { text: 'add the best one you found', testMode: true }, (e) => events.push(e), {
      baseUrl: backend.url,
    });
    ...
    const sessions = await listDiscoveryAgentSessions(film.id, { baseUrl: backend.url });
