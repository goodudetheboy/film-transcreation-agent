...
describe('createInMemoryProjectRubricStore', () => {
  it('createRubric/listRubrics round-trip, scoped per project, in creation order', async () => {
    ...
    await store.createRubric('proj-a', { name: 'Food aversion', description: 'desc', weight: 3 });
    await store.createRubric('proj-a', { name: 'Wordplay', description: 'desc2', weight: 4 });
    await store.createRubric('proj-b', { name: 'Other', description: 'desc3', weight: 2 });
  ...
  it('updateRubric patches fields, scoped per project; undefined for wrong project', async () => {
    const store = createInMemoryProjectRubricStore();
    const rubric = await store.createRubric('proj-a', { name: 'Food', description: 'd', weight: 3 });
